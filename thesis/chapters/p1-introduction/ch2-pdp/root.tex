...
%?? Historical value -- early attempts at programmability
%?? cost-first, crucial, execution + perf essential
%?? 

For historical value, and to provide some context on the design and architectural decisions of modern programmable network stacks (\cref{sec:modern-pdps}), it is important to consider early developments and advances which laid the groundwork for the \gls{acr:pdp} ecosystem as we know it.
This includes the ill-fated active networking movement, leading into the development of control plane programmability via \gls{acr:sdn}.
...
In parallel with \gls{acr:sdn}'s development, dataplane programmability has become more commonplace and performant---primarily through a mixture of novel hardware architectures and more developed software stacks on host machines.
These tools offer a useful mixture of capabilities, performance characteristics, and drawbacks to consider.
To better understand how best to use these tools in service of \gls{acr:ddn}---for building full systems built on device-local state (\cref{chap:seidr}) or the primitives for inference and online learning (\cref{chap:in-net-rl})---this section details current hardware and software dataplane architectures and their predecessors.
This also gives us cause to examine the wider context of these recent \gls{acr:pdp} developments, and why they have succeeded as compared with past research forays.

%?? data plane programmability 
%?? What tools at our disposal and how best to use them -- hardware/software architectures and capabilities.
%
%?? why? for both building primitives and full systems.
%?? Wider context -- emph importance of perf in successes of PDP's return
...
This expands on the raw tooling introduced throughout \cref{sec:modern-pdps} with a higher level of abstraction, and allows us to better understand how we can push general program logic on-path (as part of a full data and telemetry processing pipeline).
It provides us also with some insight into the limitations of bespoke designs and automatic offloading.
In-network compute's use cases will be left until the sequel (\cref{sec:in-network-compute-use-cases}).

%?? Importance of in-path
%?? Overview of the HW and SW tooling that lets us achieve e.g. in-NIC RL, pushing general programs and applications down the stack
...
These blocks are statically analysed to extract the data dependencies between sub-functions, which are then composed into a chain over the routing infrastructure.
...
%?? Lends credence to goals of pushing logic down the stack and the complexity of programs we can execute
%?? shows existing ML in PDP (overall thesis)
%?? shows the kind of new input data/state we can feed to DDN
...
It must be said that the works shown here are indicative, rather then exhaustive; benefiting the system designs in \crefrange{chap:ddos-rl}{chap:seidr} by demonstrating the new kinds of network data and state we can use as (on-path) inputs to \gls{acr:ddn} systems.
