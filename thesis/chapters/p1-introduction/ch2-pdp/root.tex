...
Finally, as it is of particular importance to this thesis and the overall goal of \gls{acr:ddn}, we shall examine \gls{acr:ml}-related use cases.
...
?? More KV~\parencite{DBLP:conf/nsdi/TakruriKAA20}

?? Katran~\parencite{katran,katran-blog} L4 load bal;ancer based on \gls{acr:xdp} -- sadly no perf numbers, though used aggressively in prod.

?? Tofino Load balance~\parencite{DBLP:conf/nsdi/BarbetteTYKMPC20}
...
?? Apache-style forwarding, accel of pub-sub archs~\parencite{DBLP:conf/conext/JepsenFMFCS20}

%?? In-network storage/caching~\parencite{DBLP:conf/eurosys/SchmidPWEP20}

?? DHTs~\parencite{DBLP:conf/ancs/HugerichSS21}
