...
In-network compute is founded on the growing idea that in-path network elements such as switches, \glspl{acr:nic}, and middleboxes can and should act as the site of complex logic to accelerate applications, participate in flow control, or to aid in network management.
%In this field
%
...
However, from the Internet's inception as ARPAnet through today, programmability of networks has increased over time to simplify the management, use, and adaptability of network infrastructure~\parencite{DBLP:journals/ccr/FeamsterRZ14}.
...
\begin{itemize}
	\item \emph{Firewalls} are a necessity for filtering unauthorised traffic at stub \glspl{acr:as}, but require line-rate lookups versus large allow- and block-lists across varied protocols. Similarly, \gls{acr:nat} solutions play a similarly important role for \gls{acr:isp} networks.
...
In multitenant environments such as data centres or cloud compute providers, clients may wish to take advantage of \gls{acr:pdp} hardware if it is present.
Consider their use cases: clients might want to put their own packet processing functions between server instances, or install offloaded logic to an attached SmartNIC or \gls{acr:fpga} to improve application performance.
Recalling the single pipeline design of the \gls{acr:psa}, it's clear that this is a very difficult resource sharing problem between \gls{acr:mat} resources, pipeline width and stages, and per-packet metadata storage.
...
Relying on only the ingress \gls{acr:rmt} to determine such routes leads to potentially inflexible packet processing chains, however.
...
Each runs its own code, and applies Unix's read-write-execute privilege model over resources, fields, tables and \texttt{extern}s to limit per-program access capabilities.
...
This approach is rather coarse-grained, and prohibits re-use of tables (increasing per-user resource costs).
In addition, restricting pipeline placement to egress-only limits program expressibility---i.e., operations such as changing the output port are illegal.

While P4 registers enable useful stateful programming, concurrent access semantics and pipeline order restrictions can make some applications difficult to express.
Equally, their implementation in any platform relies on platform-specific \texttt{extern}s even as defined by the \gls{acr:psa}, and as such their semantics and correct operation will vary on a target by target basis.
...
They differ however from \gls{acr:rmt}, having each action unit (or \emph{atom}) additionally contain a memory unit for shared state which only it may access (rather than the global, shared registers of P4).
...
Domino's restrictions are close to those of \gls{acr:ebpf} programs, adding the limitation that only one entry may be accessed per array; the compiler is responsible for building and allocating \glspl{acr:mat} and concurrently sequencing all transactions.
...
While \glspl{acr:mat} and \gls{acr:fsm} blocks may be interposed freely to express more varied programs, true flexibility is only possible at present when these blocks may be easily replaced (i.e., when targeting an \gls{acr:fpga} environment).
However, the variable and state model forces pipeline stalls to clear up hazards around concurrent \gls{acr:fsm} state accesses, leading to packet drops and sub-line-rate packet processing for some dataplane functions.
...
The ingress \gls{acr:rmt} pipeline performs feature extraction among its standard duties.
...
To enable this, custom transport protocol logic is implemented in hardware, and packets are classified and further modified at line rate with the aid of P4-\gls{acr:psa} ingress and egress pipelines.
...
Netronome \gls{acr:nfp} \gls{acr:nic} tables are reliant on the optimised DCFL~\parencite{DBLP:conf/infocom/TaylorT05} data format, potentially still limiting this capability to control plane devices.
...
\texttt{PF\_RING}~\parencite{pf-ring} was motivated by changes in the kernel to prevent \gls{acr:irq} livelock at high speeds, which were insufficient to achieve true line-rate receive.
It created a kernel-user ring buffer per-socket, where received packets are copied into all ring buffers allocated to the \gls{acr:nic} with remaining space, bypassing the network stack.
\emph{Netmap}~\parencite{DBLP:conf/usenix/Rizzo12} made use of shared kernel and user memory over ring buffers, to place userland code between the \gls{acr:nic} driver and the host networking stack for a given interface (i.e., a packet bound from the \gls{acr:nic} to the network stack would traverse kernel$\rightarrow$user$\rightarrow$kernel).
...
User and kernel code share a set of ring buffers, queues, and a large block of preallocated UMEM to pass packet-sized buffers between another in response to completions and demand, and are outwardly similar to netmap in that UMEM descriptors are passed via these queues and rings.
...
One of the strengths of the above offload technologies is that their architects are keen to seem them achieve widespread adoption, and as such they tend to be well-integrated with existing technical stacks.
...
Users specify which parts of the processing graph execute on each offload device, making this a useful tool for investigating offloading strategies (but not necessarily optimal).
Per-packet metadata is user-specified, but the compiler can infer which state must cross \gls{acr:cpu}-to-\gls{acr:nic} boundaries with the aid of key annotations.
...
iPipe assumes identical language support between the host and offload target, and while C language support is common this is not guaranteed depending on how tailored the target SmartNIC may require code to be.
However, the design is effective enough to protect underlying traffic while achieving improved latency and throughput bounds over a similar \gls{acr:dpdk} dataplane.
...
Gallium uses LLVM \gls{acr:ir} to determine read-write dependencies between variables basic-blocks of the packet processing chain's control flow graph, and account for \gls{acr:pdp} hardware's capabilities around packet reads (i.e., \gls{acr:pdp} datapaths typically can't access packet bodies past $\sim$\qty{300}{\byte}).\sidenote{SmartNICs do not have such limitations, so this is mainly a consideration for \gls{acr:psa} switches or more general depolyment. Although, such access is limited to \texttt{extern}s and may need to retrieve packet data from larger, slower blocks of \gls{acr:ram}.}
...
The main drawback is that Gallium requires some annotation to translate Click primitives into \glspl{acr:mat} as well as read or write dependencies on function parameters, otherwise the conversion to P4 means minimal additional work per target device.

\paragraph{eBPF in the network}
As a solution to the limited expression granted by the OpenFlow's action set, \emph{BPFabric}~\parencite{DBLP:conf/ancs/JouetP17} proposed that \gls{acr:ebpf} programs should be used in place of individual and explicit \gls{acr:mat} definitions.
...
Its authors proposed keeping most of the OpenFlow machinery intact---the device-controller relationship over a control plane network specifically---but instead install one \gls{acr:ebpf} program per device, encoding its entire view of the dataplane.
...
Their model iteratively runs an expanded \gls{acr:ebpf} \gls{acr:isa} rather than converting programs into a complete pipelined circuit to offer faster redesign and reinstall times: complete circuit planning takes a long time, while \gls{acr:jit} compiling an \gls{acr:ebpf} program into their new \gls{acr:isa} is pretty fast.
...
This dedicated design outperforms hosts and \gls{acr:nfp} SmartNICs in latency, but typically exhibits worse throughput.
...
This considers both horizontal splits, i.e., subdividing code into \gls{acr:ebpf} chunks using tail-calls, and vertical user$\leftrightarrow$kernel splits of code.
...
This produces compiled bytecode tailored to the observed traffic distribution, twinned with adaptive inlining of tables into fast and slow paths dependent on \gls{acr:mat} contents.
While most of this optimisation is provided as-standard when working with profiled LLVM \gls{acr:ir}, Morpheus provides specific compiler plugins to account for key features such as \gls{acr:mat} logic and match classes.
...
As is standard in Click-like approaches, functions are written as directed graphs of predefined Click-like blocks, in this case each being specifically written in a hardware description language to achieve lower \gls{acr:fpga} resource utilisation.
...
Precision is not the sole culprit here, \unit{\micro\second}-level per-packet data simply has too much volume in both raw bytes and packets to meaningfully expose outside of the device.
...
A particularly noteworthy family of measurement techniques associated with \glspl{acr:pdp} is \gls{acr:int}~\parencite{p4-int}.
...
\gls{acr:int} sinks then strip this auxiliary data from packets, collect it, and report it to the control plane; enabling finer-grained measurement of network state in loads and routes, as well as network-assisted \glspl{acr:cca} and load balancing schemes.
...
These fine-grained measurements can be added per-hop, also enabling packet paths to be traced without contacting the control plane as in past \gls{acr:pdp} approaches like \emph{NetSight}'s postcarding~\parencite{DBLP:conf/nsdi/HandigolHJMM14}.
...
Network \glspl{acr:mtu} limit the amount of per-packet state which can be added, but these 
To combat these difficulties, \emph{PINT}~\parencite{DBLP:conf/sigcomm/BasatRLAYM20} introduces aggregation and approximation  mechanisms to bound per-query values into a certain budget of bits.
Most notably, per-hop data collection is uniformly randomly divided over a packet's path, but this includes numerical approximations and the use of sketches for per-flow state on switches themselves.
...
This includes a probabilistic component such that packets with \gls{acr:int} labels are more likely to receive and carry additional metadata (limiting the effects of \gls{acr:int} on most packets).
...
To gather state, it maintains sketches of source/destination \glspl{acr:ip} and ports alongside counters, as well as useful primitives like native stateless P4 implementations of SYN cookie functions and approximate Bloom-filter-based allow/blocklists.
State is polled by the control plane which is responsible for interpreting sketches to perform higher-level logic (e.g., thresholds, significant changes), and installs detection and mitigation functions using a heuristic algorithm to ensure sufficient coverage.
...
As another example, distributed hash tables like \emph{No-hop}~\parencite{DBLP:conf/ancs/HugerichSS21} see improved lookup times from the \gls{acr:pdp} ecosystem.
...
Facebook's open-source \emph{Katran} L4 load balancer~\parencite{katran,katran-blog} makes use of \gls{acr:xdp} to co-exist on existing service nodes, offer low-disruption updates, and presents lower than userland solutions.
No performance numbers are shared, though it is reportedly used aggressively in production.
\emph{Cheetah}~\parencite{DBLP:conf/nsdi/BarbetteTYKMPC20} implements consistent load balancing for arbitrary functions at switch scale on Tofino hardware, where it achieves strong tail performance guarantees for flows even during active scaling.
...
\subsection{Transports, protocols, and routing}
...
\Textcite{DBLP:conf/im/KunzeGSWR21} find that \gls{acr:aqm} solutions must be co-designed for the target environment, for instance pipeline and register access constraints make it impossible to express some disciplines.
Implementation of \gls{acr:aqm} schemes like \emph{PIE}~\parencite{rfc8033} requires numerous tradeoffs,
with meaningful performance costs.
...
ACK messages carry their packet's \gls{acr:int} data; the sender adjusts its target rate per-ACK (minor) and per-window (major).
...
While I cover specific implications of data formats and training algorithms later (\cref{sec:numerical-representations-for-embedded-ml,sec:learning-an-approximation}), the gist is that machine learning inference and training are generally dependent on floating point data (as training gradients are real-valued), specialised accelerators accessed as peripherals like \glspl{acr:gpu} or \glspl{acr:tpu}, and are computationally complex.
...
Inference at this location thus presents real tradeoffs: either choose throughput by batching queries and new data for the \gls{acr:gpu}, or choose consistent (tail) latencies by using the \gls{acr:cpu}.
...
Equally, per-packet inference (i.e., for security, \gls{acr:qos}, or \gls{acr:te} classification) is untenable in this framework.
...
%?? batching, etc.
In-network \gls{acr:ml} research aims to solve this (preferably at line-rate), focussing on the modifications needed to install inference techniques onto \gls{acr:pdp} hardware.
%?? Also how the network can help hosts learn.
This involves tailoring algorithms and data formats to the target device: more details on suitable data representations are presented through \Cref{sec:fixed-point-and-binary}.
...
\emph{IIsy}~\parencite{DBLP:conf/hotnets/XiongZ19} investigate per-packet inference using pre-trained classical \gls{acr:ml} models on programmable switch hardware, as well as methods for cheaply updating models at runtime through the control plane.
In particular, they use custom parsers (acting as a feature extraction mechanism) and convert parameters and matching logic into \glspl{acr:mat} to implement these models using pure P4.
...
\emph{BaNaNa SPLIT} shows how these \glspl{acr:bnn}, installed in \glspl{acr:nic}, can as a partial offload mechanism~\parencite{DBLP:conf/sigcomm/SanvitoSB18}; \gls{acr:dnn} inference is often carried out on the \gls{acr:cpu} to reduce the above latencies imposed by \gls{acr:gpu} batching and transfer, but they show that the fully-connected layers of such networks can be accelerated further by \glspl{acr:nn} subject to some accuracy loss.
...
At fabrication time, Taurus may be configured to use the required floating-point depth.
Other specialised hardware includes \emph{BrainWave}~\parencite{DBLP:conf/isca/FowersOPMLLAHAG18}---an \gls{acr:npu} designed solely around heavily \gls{acr:simd} and parallelised \gls{acr:nn} inference---reduces batching by \qty{32}{\texttimes} compared to \gls{acr:gpu} acceleration.
...
Aggregating these gradients is simply vector addition, which any interim node may perform to reduce this traffic burden.
...
Some services, such as those associated with game hosting, are likely to be targetted simply for competitive advantage or reputation~\parencite{aws-shield-review-2020}.
Accordingly, they are often nicknamed an `800-pound gorilla'~\parencite{DBLP:conf/imc/CzyzKGPBK14} on the wider Internet.
...
Consider volumetric attacks on Dyn (\qty{1.2}{\tera\bit\per\second}), who at that time hosted key resources for Twitter, Spotify, and Netflix~\parencite{dyn-ddos-2016}, the web host OVH (\qty{1}{\tera\bit\per\second})~\parencite{ovh-ddos-2016}, and the Github platform (\qty{1.35}{\tera\bit\per\second})~\parencite{github-ddos-2018}.
...
Attacks may be classified on two orthogonal axes: \emph{Direct vs.~Reflection} and \emph{Volumetric vs.~Semantic}.
\begin{description}
	\item[Direct] Attackers send packets directly towards their target. Random \gls{acr:ip} spoofing is often used to make blacklisting more difficult, but leaves evidence of an attack and its characteristics due to \emph{backscatter}, visible to network telescopes~\parencite{DBLP:conf/lisa/Moore03,DBLP:conf/imc/RichterB19}.
...
\end{description}
We'll focus mainly on volumetric attacks (direct and reflection), as these are the attack vectors applied in the above case studies.
...
Scanning for \num{e5} amplifiers of a popular service can be done in minutes, making \gls{acr:ntp} particularly dangerous due to its high amplification rate.
...
\gls{acr:ntp} quickly became the attack vector of choice \cite{DBLP:conf/imc/CzyzKGPBK14}.
They find that most vulnerable amplifiers are \emph{end-hosts}, typically offering \qty{4}{\times} amplification.
At the time of publication, they observed that \gls{acr:ntp} amplification attacks had risen in volume by $\sim$\qty{1000}{\times}, though were slowly declining; of particular interest is that \qty{85}{\percent} of attacks over \qty{100}{\giga\bit\per\second} relied upon \gls{acr:ntp} reflection.
...
While there were many almost comical findings (a surfeit of \gls{acr:ntp} servers holding the incorrect time, old versions), of greatest concern was the presence of ``mega-amplifiers'' offering \SIrange{e3}{e9}{\times} amplification due to the presence of network loops.
...
While this cannot be directed to an arbitrary target, per se, this presents another vulnerability in the availability critical infrastructure that administrators must be aware of.
...
Instead, they use their bandwidth to communicate with as many legitimate or dummy servers as they can such that the outbound traffic of all participating clients.
...
Consider probabilistic packet drop at a rate $p\in\left(0,1\right]$---pushback~\parencite{DBLP:journals/ccr/MahajanBFIPS02a}---loss-ignoring and \gls{acr:cbr} traffic's send rate will scale in proportion to $1 - p$.
\gls{acr:tcp} Reno and the like exhibit greater falloff proportional to $1/\sqrt{p}$ courtesy of the Mathis equation~\parencite{DBLP:journals/ccr/MathisSMO97}, with a kinder $1/p^{0.75}$ for \gls{acr:tcp} Cubic~\parencite{rfc8312}, inflicting \emph{collateral damage} on such legitimate flows.
...
Finally, the exact proportions of this heterogeneous mixture vary over time and between \gls{acr:as} and network classes.
%?? it depends on ?? environment
%?? Heterogeneirty of traffic classes
%?? Classes respond in different ways to different actions.
Consider a point estimate of sorts obtained by analysing the 2018 \gls{acr:caida} traces~\parencite{caida-2018-passive} (\cref{adx:caida-traffic}).
...
This variability extends also to the behaviour of flows \emph{within} a protocol.
...
\subsection{Defences}
According to \textcite{DBLP:conf/imc/JonkerKKRSD17}, the most-used techniques in deployment are \emph{DDoS Protection Services}.\sidenote{?? Para before this. Mention one or two of the old-school ways brought up in the DDoS section like AIMD, Pushback...}
...
\gls{acr:ddn} solutions to \gls{acr:ddos} attacks of various kinds have been examined through the literature, such as \Textcite{DBLP:conf/lcn/BragaMP10}, \emph{Marl}~\parencite{DBLP:conf/iaai/MalialisK13,DBLP:journals/eaai/MalialisK15}, and \emph{Athena}~\parencite{DBLP:conf/dsn/LeeKSPY17}.
\Cref{sec:ddn-uses-security} explains these, alongside their drawbacks and experimental shortcomings, in detail.
Marl in particular has design flaws which are examined in great detail, motivating the improved \gls{acr:rl} work I develop in \cref{chap:ddos-rl}.
...
Finally, by definition their assumptions cannot extend to \gls{acr:cbr} traffic (e.g., \gls{acr:udp} \gls{acr:voip} traffic), which as we know from \cref{sec:ddos-contributing-factors} makes up a sizeable proportion of network traffic.
...
\emph{SENSS}~\parencite{DBLP:conf/acsac/RamanathanMYZ18} aims to help hosts and/or \emph{endpoint-servers} communicate upstream with \glspl{acr:isp}.
The rationale is that although \gls{acr:ddos} traffic can be filtered at any point along its path, it will impact less of the network if it is filtered \emph{close to its source}---this observation holds true in all attack classes (direct, reflection, \gls{acr:lfa}/crossfire), which exhibit a tree-like pattern of traffic.
This information currently propagates through human channels, eventually leading to traffic black-holing being performed by key \glspl{acr:as}.
...
Hosts and agents must perform proof-of-work challenges attached to flow cookies to become eligible for whitelisting (which is verifiable by any other node)---however, this requires some degree of re-architecting the network stacks of all hosts.
...
I have described the rich history of programmable networks from their initial forays to-date, including what is now a rich tapestry of software-defined routing and bespoke programmable devices mixed with heavily-optimised host machines.
...
It offers today source of raw device data which we would have never been able to feasibly use or export, as well as the necessary tools to perform complex data analysis at line-rate and switches' scale.
...
This may well take the form of incoming heterogeneity, as device manufacturers produce \gls{acr:pdp} devices tailored to different use cases in much the same way as \emph{Taurus}---projects like \emph{BrainWave} reiterate that network hypergiants already hold the means and motivation to do just this.