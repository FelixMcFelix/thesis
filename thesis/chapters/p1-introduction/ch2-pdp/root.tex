...
?? Stream processing and fusion (for both mgmt and apps.)
...
?? DPI?~\parencite{DBLP:conf/conext/HypoliteSHDDS20}
?? String/regex mathing?~\parencite{DBLP:conf/infocom/WangZLLLJX21}
?? compression~\parencite{DBLP:conf/conext/VaucherYFLS20}

%?? In-network storage/caching~\parencite{DBLP:conf/eurosys/SchmidPWEP20}

\subsection{Transports, Protocols, and Routing}
?? HULA routing~\parencite{DBLP:conf/sosr/KattaHKSR16}
...
?? PIFO~\parencite{DBLP:conf/sigcomm/SivaramanSACCAB16}

?? PIEO~\parencite{DBLP:conf/sigcomm/Shrivastav19} aqm

Turns out that you can't just write AQM solns in P4, you need to co-design for the target environment---with meaningful performance cost to boot, based on the tradeoffs you need to make~\parencite{Kunze-P4-AQM}.

?? HPCC~\parencite{DBLP:conf/sigcomm/LiMLZFTCZKAY19}

?? NDP~\parencite{DBLP:conf/sigcomm/HandleyRAV0AW17}

?? tHis?~\parencite{DBLP:conf/nsdi/HsuBCRW20}
...
Unfortunately, to the best of my knowledge true on-device \gls{acr:pdp} \gls{acr:ml} or \gls{acr:rl} model training hasn't yet been examined beyond the work I present in \cref{chap:in-net-rl}.
