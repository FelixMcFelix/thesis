...
These questions are the founding principles of \gls{acr:ddn}\sidenote{Alternatively titled \emph{self-driving networks}.}, a recent field of research focussed on the automatic control and optimisation of network systems, which has sprung forth due to recent advances in \gls{acr:ml} and \gls{acr:rl}.

The ideas and goals of automated network control have existed and evolved in one form or another, particularly as computational inference and learning have grown more powerful.
...
In the case of \gls{acr:rl}-based works, I highlight the state space (\rllitstate), action space (\rllitact{} if discrete, \rllitactreal{} if real-valued), and reward source (\rllitreward).
...
\textcite{DBLP:conf/hotnets/ValadarskySST17} show how \gls{acr:rl} can be used to map the last $k$ demand matrices (\rllitstate) into a set of edge weights (\rllitactreal).
The calculated weights are used as the input to compute probabilistic forwarding strategies based on classical hop-by-hop routing strategies, which then allow congestion to be computed for the following demand matrix (\rllitreward).
...
\emph{AuTO}~\parencite{DBLP:conf/sigcomm/ChenL0L18} examines the task of flow scheduling, employing \gls{acr:drl} to perform traffic optimisation, using the DDPG algorithm~\parencite{DBLP:journals/corr/LillicrapHPHETS15} to update a \gls{acr:dnn} policy.
...
To overcome the high latency of action computation via \gls{acr:nn}, two agents are trained, handling aspects of short and long flows respectively.
The first learns to optimise the flow size thresholds which define priority allocation at each switch (and accordingly learn how best to demarcate long and short flows); these short flows are routed by \gls{acr:ecmp}.
...
?? Useful things about above: refer back to long tail of network traffic, $\sim$\qty{100}{\milli\second} \qty{10}{\micro\second}. $100, 200, 300, \dots, 900$

(\rllitstate), (\rllitact/\rllitactreal), (\rllitreward).

\parencite{DBLP:conf/sigcomm/00010YC20}
...
* https://dl.acm.org/doi/10.1145/3386367.3433030 -- MPCC: online learning multipath transport (IMPORTANT)

?? Note: make sure that intra/inter-AS routing are explained elsewhere.
...
\subsection{Takeaways for effective data-driven networking}
...
Note -- are actor-critic methods needed for continuous action spaces? Might be worth mentioning whjat it takes to learn both discrete and continuous.
