...
?? Explain what's upcoming.

%\subsection{Multiagent methods}

\subsection{Distributed training}
?? Why else needed? \glspl{acr:gpu} often needed, naturally there's a per-machine limit due to \gls{acr:pcie} bus.

?? Some PS-related works?~\parencite{DBLP:journals/corr/AbadiABBCCCDDDG16,DBLP:conf/nips/DeanCMCDLMRSTYN12,DBLP:conf/osdi/LiAPSAJLSS14}

?? Data-parallel vs model parallel.

?? Parameter server... Synchronous vs Asynchronous

?? Than all-reduce (sync).

As discussed above, distributed training of neural networks is a hot-button topic.
My understanding is that the case for traditional neural networks (ML classifiers and so on) relies upon much of the same technology that is used in distributed RL training, with a lower dependency on gradient aggregation time due to the larger updates observed at each node (hence, fewer iterations of training).
For ease, I reproduce the relevant RL literature:

\begin{quote}
		Modern distributed RL may either be synchronous or asynchronous, and either uses a central parameter/processing server~\cite{DBLP:conf/nips/LiASY14} (introducing an obvious bottleneck) or the AllReduce~\cite{DBLP:conf/micro/LiPAYQPWSEK18} paradigm (more network hops) to combine policy gradient updates.
		Aggregating gradient updates consumes around \SI{83.2}{\percent} of training time for synchronous training, while gradient vectors can grow ``stale'' in the asynchronous case if they take too long to arrive at agents (harming convergence).
\end{quote}
...
Specialised optical interconnects~\parencite{DBLP:conf/sigcomm/ShirkoohiGAZGBV21}

Whatever this is~\parencite{DBLP:conf/sigcomm/ZhuangLZWLNMS21}, can accelerate e.g. the RL library Ray~\parencite{DBLP:conf/osdi/MoritzNWTLLEYPJ18}.
