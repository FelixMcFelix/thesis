...
While it's impossible to convey all of the specifics which are involved in casting a problem as an \gls{acr:mdp} which will behave effectively, the above use cases have given us insights into: (list paragraph names here.)

\paragraph{Deadline sensitivity}
...
As such, decisions taken with reduced frequency, out of the critical data path
In contrast, per-packet or per-flow inference is almost entirely restricted to end-hosts, who by construction have lower flow counts than servers and reduced traffic for consideration ($\sim$\qtyrange{10}{300}{\mega\bit\per\second}).
...
\begin{itemize}
	\item relying on a heuristic method for steady-state operation whilst taking infrequent \gls{acr:rl} or \gls{acr:ml} actions can greatly cut runtime \gls{acr:cpu} and \gls{acr:gpu} costs, while also providing more reliable or interpretable behaviour,
	...
	\item a heuristic replacement exactly matches `normal' behaviour until the policy stabilises, making such systems more suitable for from-scratch training in real networks.
\end{itemize}
...
\glspl{acr:dnn}, particularly as used by actor-critic \gls{acr:drl} algorithms, often employ two-headed networks---having a shared feature extraction component between the policy and value networks.
