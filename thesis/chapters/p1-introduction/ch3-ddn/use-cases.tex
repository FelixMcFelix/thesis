...
As discussed and introduced earlier in \cref{sec:pdp-uses-transports} (and as a motivating example in the \cref{chap:intro}), the design of effective \glspl{acr:cca} very much remains an open topic.
...
\emph{Stick}~\parencite{DBLP:conf/infocom/HuangZZW0S20} uses the DDPG \gls{acr:drl} algorithm to train a smaller \gls{acr:cnn} to provide the target buffer occupancy allocated for heuristic, buffer-based chunk selection methods \prllitactreal{}.
