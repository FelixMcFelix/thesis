...
In the event of either this scenario or (as above) some gradual change in the underlying problem, it's reasonable to want to tailor an existing policy to suit our current needs.
Alternatively, we might aim to prevent performance degradation arising from complex dynamics we don't yet know how to model, but can see in live networks.
...
\begin{itemize}
	...
	This relies on two protocol-agnostic, flow-granularity \gls{acr:rl} agent designs (\emph{Instant} and \emph{Guarded} action models), alongside algorithmic modifications to Sarsa to enable better concurrent learning from many in-progress \gls{acr:rl} trajectories, and reward functions tailored to detecting the negative impacts of amplification \gls{acr:ddos} and \glspl{acr:lfa}.
	...
	I present an analysis of why \gls{acr:rl} in \gls{acr:pdp} hardware is needed and best-placed to interact with the network, made feasible by classical \gls{acr:rl} methods and quantisation.
	To make this task feasible, I design an \gls{acr:rl} interaction model based on path-adjacent compute to protect carried traffic, offer an analysis of suitable data formats for online \gls{acr:ddn} in resource-constrained hardware.
	I then provide a proof that 1-step temporal-difference \gls{acr:rl} algorithms admit a parallelisable, map-reduce form with tile-coded policies, culminating in \emph{ParSa}, a wait-free, parallel, online \gls{acr:rl} algorithm to accelerate tile-coded policy inference and updates.
	Exploring the design space, I present parallel \gls{acr:rl} strategies tailored to provide either maximum offline throughput, or optimal state-action latencies and online throughput, and work allocation algorithms and communication tailored to SmartNIC devices with an explicitly tiered memory model.
	I show \approachshort{}'s effectiveness via in-depth evaluation of how it affects carried dataplane traffic, performs in latency and throughput under different policy sizes (simple and complex state), and improves on host machines.
