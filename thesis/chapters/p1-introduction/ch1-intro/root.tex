...
Today's Internet is a prime example.
...
Owing to this complexity, keeping the `Internet machinery' well-oiled and performant is a difficult task.
As these networks have grown larger over the last half-century, become more flexible to do just this... (why? larger? more clients?) Control plane, \gls{acr:sdn}. ?? step away from limits and hard, fixed impls? ?? avoid being shackled to archaic design choices
?? need a pdp problem
many aspects of networks complex, heuristic, hand-tuned CCAs etc. operating on limited information
This is embodied by the \glspl{acr:cca} which underpin the majority of Internet traffic---
?? effectively reverse-engineering network state by a proxy metric
?? while this is impressive...
What happens as networks change?
?? infeasible to hand-tune per workload, per topology.
?? better info? and machines can do better.
