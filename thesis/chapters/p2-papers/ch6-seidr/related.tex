...
% SONATA paper is relevant~\parencite{DBLP:conf/sigcomm/GuptaHCFRW18}.
...
% Several tools analyse packet traces such as pcaps to find performance limitations~\parencite{zhang2002characteristics}. These methods usually work on offline traces and do not provide (near)-real-time analysis.
% Wireshark~\parencite{wireshark} is one of the most well-known applications of this class, known for its ability to parse and analyse many protocols for anomalies and violations by allowing extensions from the community.
