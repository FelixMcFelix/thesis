...
%
%?? Introduce tile coding and whatever as the soln first for compute complexity etc.
%
%?? old, salesman-y
...
Recall that the aggregation step involves \emph{only additions}, that atomic fetch-add instructions are commonly offered on many machine architectures,\sidenote{While other datatypes such as \mintinline{rust}{f32} can technically support atomic operations on many platforms, outside of specialised \gls{acr:gpu} environments such as CUDA these invoke non-trivial performance costs due to cache and register placement restrictions.} and that fixed-point addition is exactly integer addition.
...
Parallelising the \gls{acr:rl} process among processes in this way thus requires tight coupling between the function approximation and \gls{acr:rl} update algorithm.
...
However, this is a deployment-specific implementation detail, as in addition to raw task costs an effective scheduler must account for, e.g., cache size limits and physical/logical core differences.
...
An \emph{Act} request is then sent to all minion threads using (\cref{alg:parsa:parctl-act}).
While the minions produce the action preference list asynchronously, the control thread then requests an output message slot and prepares it by copying the state into the requisite buffer (\cref{alg:parsa:parctl-preprep}).\sidenote{In principle, the controller thread may also act as another minion after this step, between scatter and recv calls, subject to code store limits.}
...
The index of the largest value is then taken to be the given action, though this may be trivially modified for $\epsilon$-greedy selection (\cref{alg:parsa:parctl-acchoose}), which is then placed into the output message and returned to the environment (\cref{alg:parsa:parctl-acemit}).
...
If a match is found, it computes and reduces by $\alpha$ the Sarsa \gls{acr:td} value $\delta_t$ (\crefrange{alg:parsa:parctl-td1}{alg:parsa:parctl-td2}), before passing this adjustment and the prior state action pair to minion threads to perform the update step (\crefrange{alg:parsa:parctl-update}{alg:parsa:parctl-await2}).
The newest state-action-value tuple is then stored (\cref{alg:parsa:parctl-store}).

The \emph{Minion} procedure processes a single request to retrieve and aggregate, or update, the action values learned for an input state over some subset \emph{tasks}.
...
As a result, this results in a larger \texttt{memcpy} in \emph{Ctl} (the serial portion of ParSa), and as such it is more efficient to recompute hit indices in the update step.
...
This lowers the state-action latency and produces a corresponding increase to throughput in tandem.
However, the factor of speedup is \emph{worse} than $n$~\unit{\times}, and exhibits diminishing returns in terms of core count---for instance due to the cost of \texttt{memcpy}s and arithmetic in the serial \emph{Ctl}, message passing costs at the broadcast and receive steps, and any memory fences or reorderings caused by the compiler due to atomic arithmetic.
This is bounded again by the number of subtasks which can be generated for a given policy.
While I use tile granularity here, this may be extended to individual actions at the cost of significant repetition of work and \gls{acr:ipc} overheads dominating.
Since these aspects are platform-dependent, this behaviour can only be shown empirically as I do in \cref{sec:opal-evaluation}.\sidenote{?? Can I get a more precise cref here?!}