...
This confirms one of the key assertions in my thesis statement, namely that ?? DIRECT QUOTE HERE ONCE THIS IS READY FROM INTRO ??.
The key to doing so was to consider the architectural strengths and weaknesses of the target environment---in this case, SmartNIC devices---to creatively choose data formats and algorithms which suit their weaker, resource-constrained, \gls{acr:fpu}-free environment rather than the state of the art.
...
We have examined two concrete implementations of \approachshort{}---\indfw{} and \coopfw---which apply SmartNICs' parallelism in different ways to optimise state-action latency and online/offline throughput tailored according to operators' needs.
