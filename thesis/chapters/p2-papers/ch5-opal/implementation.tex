...
\begin{algorithm}
	...
	\sched{work\_items}{
		...
		\For{\emph{ctx} \In \emph{ctx\_costs}[..\emph{alloc\_spill}]}{
			\emph{ctx.max\_items} += 1\;
		}
		\For{\emph{ctx} \In \emph{ctx\_costs}}{
			\Let \emph{penalise\_early\_ctx} = \emph{ME\_CTXS}[\emph{me\_id}(\emph{ctx})] - \emph{local\_ctx\_id}(\emph{ctx}) - 1\;
			\emph{me\_costs}[\emph{me\_id}(\emph{ctx})]\emph{.max\_items} += \emph{ctx.max\_items}\;
			\emph{ctx.cost} = \emph{penalise\_early\_ctx}\;
			\emph{me\_costs}[\emph{me\_id}(\emph{ctx})]\emph{.cost} += \emph{ctx.cost}\;
		}
		\tcp{Sort MEs by cost / \emph{ME\_CTXS}[\emph{me}], breaking ties on largest id.}
		\Let \emph{me\_heap} = \emph{min\_heap}(\emph{me\_costs})\;
		\tcp{Sort CTXs by cost, breaking ties on smallest id.}
		\Let \emph{ctx\_heaps} = [\emph{min\_heap}(\emph{ctx\_costs}[\emph{first\_ctx}(\emph{me})..\emph{ME\_CTXS}[me]]) \Forkw \emph{me \In 0..\emph{ME\_CTXS}.len()}]\;
		\For{item \In work\_items.reverse()}{
